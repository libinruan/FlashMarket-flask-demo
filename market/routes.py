...
def register_page():
    ...
    if form.errors != {}:  # if there are no errors from the validation
        for err_msg in form.errors.values():
            flash(f'There exist error when registering new user: {err_msg}', category='danger')
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and attempted_user.check_password_correction(
            ...
            flash('Username and password are not matched! Pleae try again', category='danger')
...
def logout_page():
    logout_user()
    flash("You have been logged!", category='info')
